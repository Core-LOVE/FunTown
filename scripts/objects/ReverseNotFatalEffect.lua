...
---@overload fun(...) : MyEffect
local MyEffect, super = Class(Object)

function MyEffect:init(texture, x, y, after)
    super.init(self, x, y)
...
function MyEffect:onAdd(parent)
    super.onAdd(self, parent)

    self.start_color = self.color
end

function MyEffect:update()
    self.red_timer = self.red_timer + DTMULT
    self.color = Utils.mergeColor(self.start_color, {1, 1, 1}, self.red_timer / 20)
...
function MyEffect:draw()
    local r, g, b, a = self:getDrawColor()

    for i = 0, self.blocks_x do
        for j = 0, self.blocks_y do
            local block = self.blocks[i][j]
            Draw.setColor(r, g, b, a * (1 - math.abs(block.speed / 12)))
